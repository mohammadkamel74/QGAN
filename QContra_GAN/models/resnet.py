...
class Generator(nn.Module):
    ...
    def __init__(self, z_dim, img_size, channel, g_conv_dim, g_spectral_norm, attention, attention_after_nth_gen_block,
        ...
        if g_spectral_norm:
            self.conv2d5 = snconv2d(in_channels=self.out_dims[-1], out_channels=channel, kernel_size=3, stride=1, padding=1)
            print("Conv5 with SN")
        else:
            self.conv2d5 = conv2d(in_channels=self.out_dims[-1], out_channels=channel, kernel_size=3, stride=1, padding=1)
...
class Discriminator(nn.Module):
    ...
    def __init__(self, img_size, d_conv_dim, d_spectral_norm, attention, attention_after_nth_dis_block, conditional_strategy,
                 hypersphere_dim, num_classes, nonlinear_embed, normalize_embed, channel, needs_init, mixed_precision):
...
class MidQuaternionGenBlock(nn.Module):
    def __init__(self, in_channels, out_channels, g_spectral_norm, activation_fn, conditional_bn, num_classes):
        ...
        if g_spectral_norm:
            self.conv2d0 = midqsnconv2d(in_channels=in_channels, out_channels=out_channels, kernel_size=1, stride=1, padding=0)
            self.conv2d1 = midqsnconv2d(in_channels=in_channels, out_channels=out_channels, kernel_size=3, stride=1, padding=1)
            self.conv2d2 = midqsnconv2d(in_channels=out_channels, out_channels=out_channels, kernel_size=3, stride=1, padding=1)
        else:
            self.conv2d0 = qconv2d(in_channels=in_channels, out_channels=out_channels, kernel_size=1, stride=1, padding=0)
            self.conv2d1 = qconv2d(in_channels=in_channels, out_channels=out_channels, kernel_size=3, stride=1, padding=1)
            self.conv2d2 = qconv2d(in_channels=out_channels, out_channels=out_channels, kernel_size=3, stride=1, padding=1)
...
class MidQuaternionGenerator(nn.Module):
    ...
    def __init__(self, z_dim, img_size, channel, g_conv_dim, g_spectral_norm, attention, attention_after_nth_gen_block,
        ...
        if g_spectral_norm:
            self.conv2d5 = midqsnconv2d(in_channels=self.out_dims[-1], out_channels=3, kernel_size=3, stride=1, padding=1)
            print("Conv5 with SN")
        else:
            self.conv2d5 = qconv2d(in_channels=self.out_dims[-1], out_channels=3, kernel_size=3, stride=1, padding=1)
...
class MidQuaternionDiscOptBlock(nn.Module):
    def __init__(self, in_channels, out_channels, d_spectral_norm, activation_fn):
        super(MidQuaternionDiscOptBlock, self).__init__()
        self.d_spectral_norm = d_spectral_norm
...
class MidQuaternionDiscriminator(nn.Module):
    ...
    def __init__(self, img_size, d_conv_dim, d_spectral_norm, attention, attention_after_nth_dis_block, conditional_strategy,
        ...
        d_in_dims_collection = {"32": [channel] + [d_conv_dim*2, d_conv_dim*2, d_conv_dim*2],
                                "64": [channel] +[d_conv_dim, d_conv_dim*2, d_conv_dim*4, d_conv_dim*8],
                                "128": [channel] +[d_conv_dim, d_conv_dim*2, d_conv_dim*4, d_conv_dim*8, d_conv_dim*16],
                                "256": [channel] +[d_conv_dim, d_conv_dim*2, d_conv_dim*4, d_conv_dim*8, d_conv_dim*8, d_conv_dim*16],
                                "512": [channel] +[d_conv_dim, d_conv_dim, d_conv_dim*2, d_conv_dim*4, d_conv_dim*8, d_conv_dim*8, d_conv_dim*16]}
        ...
        for index in range(len(self.in_dims)):
            if index == 0:
                self.blocks += [[MidQuaternionDiscOptBlock(in_channels=self.in_dims[index],
                                              out_channels=self.out_dims[index],
                                              d_spectral_norm=d_spectral_norm,
                                              activation_fn=activation_fn)]]
            else:
